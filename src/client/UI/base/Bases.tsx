...
export namespace Animated {
  ...
  export class TypeWriter extends Component<TWProps, { render: boolean }> {
    public Retype() {
      ...
      this.mo.setGoal(
        new Spring(1, {
          frequency: 20 / this.props.Text.size(),
          dampingRatio: 0.8,
        }),
      );
    ...
    render() {
      ...
      g.Text = this.b.map((v) => {
        return this.props.Text.sub(1, math.ceil(this.props.Text.size() * TweenService.GetValue(v, "Quad", "InOut")));
      });
      g.BackgroundTransparency = this.fb.map((v) => {
        if (this.props.BackgroundTransparency !== undefined) return 1 + (this.props.BackgroundTransparency - 1) * v;
        return 1 - v;
      });
